...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/trust-lane", label: "Trust Lane" },
    { href: "/fleet-risks", label: "Fleet Risks" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Apple Device Trust Control Plane</div>
        <div class="right">
          <div>fleet posture · synthetic apple device packets</div>
          <div>macos · ios · ipados · endpoint ops</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            <span class="meta-chip">Wave 13 · Device Governance</span>
            <span class="meta-chip">Apple endpoint proof</span>
            <span class="meta-chip">Synthetic MDM export packets</span>
          </div>
          <h1>Apple device trust, stale check-in drift, and remediation posture that stay operator-readable.</h1>
          <p>This control plane turns normalized Apple fleet exports into a buyer-readable endpoint posture surface for platform, security, and IT operations teams: noncompliant devices, stale check-in risk, encryption gaps, personal-device review, and the remediation packet needed before access and rollout windows break.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Enterprise Apple endpoint trust for security, platform, and IT teams.</strong><br />Audit-safe fleet visibility, remediation sequencing, and personal-device review without tenant-secret exposure.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo includes a reusable analyzer that reads normalized device-export packets and turns them into trust posture, risk, and remediation packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Apple device management / macOS / iOS / endpoint security</strong> should see real endpoint operations work: device trust, stale check-in drift, encryption policy, and personal-device controls.</p>
          </div>
        </aside>
      </div>
      ${body}
...
function severityClass(value: string) {
  if (value === "high" || value === "red" || value === "noncompliant") {
    return "red";
  }
  if (value === "medium" || value === "yellow" || value === "inGracePeriod") {
    return "yellow";
  }
  if (value === "green" || value === "compliant") {
    return "green";
  }
  return "info";
}
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">device export lens</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.devices}</div><div class="lbl">managed devices</div><div class="h">Synthetic macOS, iOS, and iPadOS records across executive, finance, seller, and frontline lanes.</div></div>
          <div class="kpi green"><div class="v">${metrics.healthyDevices}</div><div class="lbl">healthy devices</div><div class="h">Devices currently remaining in compliant posture.</div></div>
          <div class="kpi red"><div class="v">${metrics.noncompliantDevices}</div><div class="lbl">noncompliant devices</div><div class="h">Devices already blocking or close to blocking governed access.</div></div>
          <div class="kpi amber"><div class="v">${metrics.staleDevices}</div><div class="lbl">stale check-ins</div><div class="h">Devices whose last check-in has drifted beyond the stale threshold.</div></div>
          <div class="kpi plum"><div class="v">${metrics.personalDevices}</div><div class="lbl">personal scope</div><div class="h">Personal Apple devices under corporate trust policy and review pressure.</div></div>
          <div class="kpi red"><div class="v">${metrics.highFindings}</div><div class="lbl">high findings</div><div class="h">High-severity endpoint issues that need operator intervention first.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">device trust · stale check-in · remediation packet</div></div>
        <div class="stack">
...
export function renderTrustLane() {
  return layout(
    "Apple Device Trust Console — Trust Lane",
    "/trust-lane",
    `<section class="section">
        <div class="sh"><h2>Trust Lane</h2><div class="note">owner · device class · trust posture</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Device</th>
          ...
          <tbody>
            ${trustLane()
              .map(
                (lane) => `<tr>
                    <td><b>${lane.deviceName}</b><br />${lane.lane}<br />${lane.businessRole}</td>
                    <td>${lane.owner}</td>
                    <td><span class="st ${severityClass(lane.severity)}">${lane.complianceState}</span></td>
                    <td>${lane.operatingSystem}</td>
...
export function renderFleetRisks() {
  return layout(
    "Apple Device Trust Console — Fleet Risks",
    "/fleet-risks",
    `<section class="section">
        <div class="sh"><h2>Fleet Risks</h2><div class="note">severity · owner · device</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Risk</th>
            ...
            ${fleetRisks()
              .map(
                (finding) => `<tr>
                    ...
                    <td>${finding.owner}</td>
                    <td>${finding.deviceName ?? finding.deviceId}<br />${finding.user ?? "—"}</td>
                    <td>${finding.operatingSystem}<br /><span>${finding.complianceState}</span></td>
...
export function renderRemediationPosture() {
  return layout(
    "Apple Device Trust Console — Remediation Posture",
    "/remediation-posture",
    `<section class="section">
        <div class="sh"><h2>Remediation Posture</h2><div class="note">packet readiness · blocker · fleet window</div></div>
        <div class="board">
          ${remediationPosture()
            .map(
...
export function renderVerification() {
  return layout(
    "Apple Device Trust Console — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real Apple endpoint operations posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Apple Device Trust Console — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/trust-lane</code>, <code>/fleet-risks</code>, <code>/remediation-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/trust-lane</code>, <code>/api/fleet-risks</code>, <code>/api/remediation-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline device-export analysis</div><p><code>npx apple-device-trust fixtures/devices-clean.json --format summary</code> renders the same underlying analyzer posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
