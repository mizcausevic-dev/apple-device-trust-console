...
function New-ProofImage {
    ...
    $graphics.DrawString("Apple Device Trust Console", $eyebrowFont, $accentBrush, 92, 92)
...
New-ProofImage -Path (Join-Path $screenshots "01-overview-proof.png") `
    ...
    -Subtitle "Apple fleet posture, stale check-in drift, personal-device review, and remediation sequencing in one operator surface." `
    -Bullets @(
        "Noncompliant Apple devices stay visible before they break access windows.",
        "Integrity exceptions and encryption gaps are raised as operator-grade findings.",
        "Endpoint posture is tied to a real remediation packet, not generic copy."
    )

New-ProofImage -Path (Join-Path $screenshots "02-trust-lane-proof.png") `
    -Title "Trust lane" `
    -Subtitle "Each Apple device lane keeps owner, business role, trust state, and next action visible." `
    -Bullets @(
        "Fleet ownership stays readable.",
        "Executive, finance, seller, and frontline lanes stay separated cleanly.",
        "Next actions remain operator-safe and audit-readable."
    )

New-ProofImage -Path (Join-Path $screenshots "03-fleet-risks-proof.png") `
    -Title "Fleet risks" `
    -Subtitle "Findings map severity, owner, device, and the exact trust rule that fired." `
    -Bullets @(
        "High-severity endpoint findings surface first.",
        "Operators can tie risk back to device role and ownership.",
        "The lane is grounded in normalized Apple device-export packets."
    )
...
New-ProofImage -Path (Join-Path $screenshots "04-remediation-posture-proof.png") `
    ...
    -Bullets @(
        "Mac, iPhone, iPad, and frontline packets stay visible.",
        "Red/yellow/green posture remains easy to scan.",
        "The system is shaped for real Apple endpoint operations proof."
    )
